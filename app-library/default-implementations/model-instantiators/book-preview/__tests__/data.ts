import { InstantiateBookPreviewModelParameters } from "../BookPreviewModelInstantiator";

export const modelInstantiatorTestInput: InstantiateBookPreviewModelParameters =
	{
		id: "art-image-id_2000",
		title: "Sample title",
		author: "Sample author",
		bookLink: new URL("https://sample.lnk"),
		cover: {
			source: "/sample-covers/pic.jpg",
			alt: "This is a book cover",
			placeholder:
				"data:image/png;base64,",
		},
	};
